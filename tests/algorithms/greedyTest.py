...
def exactChange41():
	...
	assert 4 in static.getNumbersFrom(outputOf(stdinArgs=[0.41]))

@passed(noBreakAndImport, hide=False)
def exactChange001():
	"""input of 0.01 yields output of 1"""
	assert 1 in static.getNumbersFrom(outputOf(stdinArgs=[0.01]))


@passed(noBreakAndImport, hide=False)
def exactChange16():
	"""input of 1.6 yields output of 7"""
	assert 7 in static.getNumbersFrom(outputOf(stdinArgs=[1.6]))


@passed(noBreakAndImport, hide=False)
def exactChange420():
	"""input of 4.2 yields output of 18"""
	assert 18 in static.getNumbersFrom(outputOf(stdinArgs=[4.2]))


@passed(noBreakAndImport, hide=False)
def handlesWrongInput():
	"""rejects negative input, then accepts an input of 0.41"""
	assert 4 in static.getNumbersFrom(outputOf(stdinArgs=[-1, -1, 0.41]))